...
def test_add_iteration():
    ...
    for i in range(len(nets)):
        ...
        cndp.add_data([i * 0.1, i**2], np.array([[i, 2-i], [i**2, 0]]))
        cndps_p.append(cndp)
    ...
    assert len(rh.it_hist[0][0].ff) == 2
    assert rh.it_hist[0][0].ff[0] == pytest.approx(0)
    assert rh.it_hist[0][0].ff[1] == pytest.approx(0)
    assert np.array_equal(rh.it_hist[0][0].conf_mat, np.array([[0, 2], [0, 0]]))

    assert_chaos_networks_same(rh.it_hist[0][1].net, nets[1])
    assert len(rh.it_hist[0][1].ff) == 2
    assert rh.it_hist[0][1].ff[0] == pytest.approx(0.1)
    assert rh.it_hist[0][1].ff[1] == pytest.approx(1)
    assert np.array_equal(rh.it_hist[0][1].conf_mat, np.array([[1, 1], [1, 0]]))

    assert_chaos_networks_same(rh.it_hist[0][2].net, nets[2])
    assert len(rh.it_hist[0][2].ff) == 2
    assert rh.it_hist[0][2].ff[0] == pytest.approx(0.2)
    assert rh.it_hist[0][2].ff[1] == pytest.approx(4)
    assert np.array_equal(rh.it_hist[0][2].conf_mat, np.array([[2, 0], [4, 0]]))

    assert_chaos_networks_same(rh.it_hist[0][3].net, nets[3])
    assert len(rh.it_hist[0][3].ff) == 2
    assert rh.it_hist[0][3].ff[0] == pytest.approx(0.3)
    assert rh.it_hist[0][3].ff[1] == pytest.approx(9)
    assert np.array_equal(rh.it_hist[0][3].conf_mat, np.array([[3, -1], [9, 0]]))

    assert_chaos_networks_same(rh.it_hist[0][4].net, nets[4])
    assert len(rh.it_hist[0][3].ff) == 2
    assert rh.it_hist[0][4].ff[0] == pytest.approx(0.4)
    assert rh.it_hist[0][4].ff[1] == pytest.approx(16)
    assert np.array_equal(rh.it_hist[0][4].conf_mat, np.array([[4, -2], [16, 0]]))

    # rh.it_hist[1] test
    assert_chaos_networks_same(rh.it_hist[1][0].net, nets[0])
    assert len(rh.it_hist[1][0].ff) == 2
    assert rh.it_hist[1][0].ff[0] == pytest.approx(0)
    assert rh.it_hist[1][0].ff[1] == pytest.approx(0)
    assert np.array_equal(rh.it_hist[1][0].conf_mat, np.array([[0, 2], [0, 0]]))

    assert_chaos_networks_same(rh.it_hist[1][1].net, nets[2])
    assert len(rh.it_hist[1][1].ff) == 2
    assert rh.it_hist[1][1].ff[0] == pytest.approx(0.2)
    assert rh.it_hist[1][1].ff[1] == pytest.approx(4)
    assert np.array_equal(rh.it_hist[1][1].conf_mat, np.array([[2, 0], [4, 0]]))

    assert_chaos_networks_same(rh.it_hist[1][2].net, nets[1])
    assert len(rh.it_hist[1][2].ff) == 2
    assert rh.it_hist[1][2].ff[0] == pytest.approx(0.1)
    assert rh.it_hist[1][2].ff[1] == pytest.approx(1)
    assert np.array_equal(rh.it_hist[1][2].conf_mat, np.array([[1, 1], [1, 0]]))

    assert_chaos_networks_same(rh.it_hist[1][3].net, nets[4])
    assert len(rh.it_hist[1][3].ff) == 2
    assert rh.it_hist[1][3].ff[0] == pytest.approx(0.4)
    assert rh.it_hist[1][3].ff[1] == pytest.approx(16)
    assert np.array_equal(rh.it_hist[1][3].conf_mat, np.array([[4, -2], [16, 0]]))

    assert_chaos_networks_same(rh.it_hist[1][4].net, nets[3])
    assert len(rh.it_hist[1][4].ff) == 2
    assert rh.it_hist[1][4].ff[0] == pytest.approx(0.3)
    assert rh.it_hist[1][4].ff[1] == pytest.approx(9)
    assert np.array_equal(rh.it_hist[1][4].conf_mat, np.array([[3, -1], [9, 0]]))

    cndps_p[0].ff = [111]
    assert len(cndps_p[0].ff) == 1
    assert cndps_p[0].ff[0] == 111
    assert cndps[0].ff[0] == 111
    assert rh.it_hist[0][0].ff[0] == pytest.approx(0)
    assert rh.it_hist[1][0].ff[0] == pytest.approx(0)

    rh.it_hist[0][1].ff = [222]
    assert len(rh.it_hist[0][1].ff) == 1
    assert rh.it_hist[0][1].ff[0] == pytest.approx(222)
    assert rh.it_hist[1][2].ff[0] == pytest.approx(0.1)
